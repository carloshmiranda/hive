...
- `[code]` 2026-04-04 — **QStash Flow Control + Unified Dispatcher — Phases 1–7 complete (branch hive/improvement/audit-and-docs)** — 7-phase system overhaul: (1) Unified `/api/dispatch/work` as single dispatch authority with priority scoring and health gate integration. (2) Event-driven dispatch — all 6 webhook/trigger routes call dispatch/work directly (60s latency vs 4h Sentinel polling). (3) QStash Flow Control — `flowControl: { key: "engineer", parallelism: 1 }` replaces DB polling for engineer parallelism. (4) pgvector semantic search in `/api/agents/context` — cosine similarity on task_description embeddings, MMR as cold-start fallback; returns `playbook_search_method: "vector"|"mmr"`. (5) Recursive quality loop — CEO review grades agent_actions → `quality_score` on agent_actions → routing_weights → Redis mirror → distillation endpoint (`/api/distill/trajectory`) extracts patterns → playbook entry with embedding. (6) Edge Config thresholds — 5 numeric limits moved from hardcoded to configurable via `getThreshold()`. Redis playbook leaderboard (ZADD). Interactive session claim lock (`SET NX EX 7200 claim:{id}`). (7) Health-gate SQL CTE refactor (ghost-failure dedup), `safeInsertBacklog()` dedup helper, QSTASH_TOKEN warning, cycle-complete error logging, ARCHITECTURE.md + DECISIONS.md (ADR-041–045) + MISTAKES.md updated. Schema migration 014: `quality_score NUMERIC(4,3)` on agent_actions, `evolution_type TEXT` + `source TEXT` on playbook. **Pending:** run migration 014 on Neon, merge PR, set dispatch_paused=false in Edge Config to re-enable loop.
