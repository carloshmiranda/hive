...
**Date:** 2026-03-27
**Status:** accepted
**Context:** Backlog was managed with dual-sync: DB (`hive_backlog`) as source of truth + `BACKLOG.md` auto-generated from DB. Problems: (1) `fs.writeFile` silently fails on Vercel's read-only FS — BACKLOG.md never actually synced in production, (2) agents editing BACKLOG.md had changes overwritten by next regen, (3) two-way sync was fundamentally broken.
**Decision:** `hive_backlog` DB is the only source of truth. BACKLOG.md is a read-only auto-generated snapshot committed via GitHub Contents API. All agents interact with the backlog via: MCP tools (Claude Code sessions), `/api/agents/backlog` endpoint with OIDC auth (GitHub Actions workflows), or `/api/backlog` (dashboard command bar). No agent reads or writes BACKLOG.md.
**Alternatives considered:**
- Fix fs.writeFile with GitHub API: still has the two-way sync problem — which direction wins on conflict?
- Remove BACKLOG.md entirely: loses human-readable snapshot for git history and quick reference
- Use GitHub Issues as backlog: adds external dependency, harder to query programmatically, loses priority/theme/category structure
**Consequences:** Zero drift between DB and file. Agents never blocked by file conflicts. BACKLOG.md exists only for human convenience — auto-generated by `regenerateBacklogMd()` in sentinel-janitor. New `/api/agents/backlog` endpoint (OIDC-authenticated) for workflow agents to create/update items.

### ADR-038: GitHub Issues as canonical work tracker
**Date:** 2026-03-28
**Status:** accepted
**Context:** Work tracking had no human-facing surface beyond the dashboard. Backlog items and company tasks existed only in Neon DB, invisible to standard developer workflows. Engineers working on company repos couldn't see what tasks existed or link PRs to work items. Carlos couldn't browse work outside the dashboard.
**Decision:** GitHub Issues become the canonical human-facing work tracker. Two-layer architecture: (1) DB (`hive_backlog` + `company_tasks`) retains operational metadata (dispatch_id, timing, priority scores), (2) GitHub Issues provide visibility, PR linking, and standard developer workflow. Hive backlog items create Issues in `carloshmiranda/hive`. Company tasks create Issues in the company's own repo (e.g., `carloshmiranda/verdedesk`). Issues are created fire-and-forget on item creation. Status sync: DB→GitHub on transitions (labels update, close on done). GitHub→DB via Sentinel polling for merged PRs with `Fixes #N` references.
**Alternatives considered:**
- GitHub Issues as sole source of truth: loses structured queries, priority scoring, dispatch metadata — DB still needed for orchestration
